...
export default function LandingPagePreview() {
  ...
  useEffect(() => {
    ...
    // Simulamos que recibimos estos datos desde alguna parte (puedes reemplazar esto)
    setProductImages(["/placeholder.png", "/placeholder2.jpg"]); // Imágenes simuladas
    setProductTitle("Producto Ejemplo");
  ...
  const clearLandingData = () => {
    ...
    setLandingData(null);
    toast({
      title: "Landing limpia",
      description: "Puedes generar una nueva versión.",
    });
  };
  ...
  return (
    <div className="w-full bg-white rounded-lg shadow-sm p-6">
      ...
      {/* Inputs de prueba (opcional, puedes borrarlos si los datos vienen por props) */}
      ...
      {landingData ? (
        <>
          ...
          <div className="mt-6 flex justify-center">
            <button
              onClick={clearLandingData}
              className="px-4 py-2 bg-red-100 hover:bg-red-200 text-red-700 rounded-md transition"
            >
              Limpiar y regenerar
            </button>
          </div>
